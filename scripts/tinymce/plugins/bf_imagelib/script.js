...
function getresizedImage () {
    ...
    if (resize.checked) {
        resizedImage = window.parent.buildfire.imageLib.resizeImage(imageUrl, {
         ...
         widgetImageExpression = '${context.sdk.imageLib.resizeImage(\'' + imageUrl + '\', { size: \'' + size + '\', aspect: \'1:1\' })}';
    } else {
        resizedImage = window.parent.buildfire.imageLib.cropImage(imageUrl, {
            size: size,
            aspect: cropAspectRatio,
        });
        widgetImageExpression = '${context.sdk.imageLib.cropImage(\'' + imageUrl + '\', { size: \'' + size + '\', aspect: \'' + cropAspectRatio + '\'' + '})}';
    }
    let img = `<img width="${
...
window.addEventListener('message', function (event) {
    ...
    if (resize.checked) {
        imageData.widgetImageExpression = '${context.sdk.imageLib.resizeImage(\'' + imageUrl + '\', { size: \'' + imageSize + '\', aspect: \'1:1\' })}';
    } else {
        imageData.widgetImageExpression = '${context.sdk.imageLib.cropImage(\'' + imageUrl + '\', { size: \'' + imageSize + '\', aspect: \'' + cropAspectRatio + '\'' +  '})}';
    }
